...
###
###     VIEW PREDICTION NETWORK MODULE
###
...
###
###     SURGERY PREDICTION NETWORK MODULE
###
...
def read_image_file(file_name, crop):
    ...
    elif file_name[-3:].lower() in img_types:
...
def get_grad_filename(image_file):
    """
    Creates a new file name (replaces extension with .png)
    :param image_file: image file name
    :return: png image file name
    ...
    new_filename = '.'.join(image_file.split('.')[:-1]) + '-GradCAM.png'
    return new_filename
...
def main():
    ...
    while True:  # Event Loop
        ...
        if event == 'OK':
            ...
            sag_filename_id = os.path.join(outdir,get_grad_filename(sag_file))
            trans_filename_id = os.path.join(outdir,get_grad_filename(trans_file))
