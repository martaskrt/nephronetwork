...
def get_metric_results(data, metric, early_stop_epoch):
    ...
    for fold in range(1, 6):
        max_avg_epochs_results['train'] += data[fold]['train'][metric][max_of_avg_epochs]
        max_avg_epochs_results['val'] += data[fold]['val'][metric][max_of_avg_epochs]
        max_avg_epochs_results['test'] += data[fold]['test'][metric][max_of_avg_epochs]
...
def compute_results(filename, data, early_stop_epoch):
    ...
    auc = get_metric_results(data, 'AUC', early_stop_epoch)
    auprc = get_metric_results(data, 'AUPRC', early_stop_epoch)
    print('{:.3f}/{:.3f}\t{:.3f}/{:.3f}\t{:.3f}/{:.3f}'.format(auc['train'], auprc['train'], auc['val'], auprc['val'],
                                                               auc['test'], auprc['test']))
    print("**********************************************************************************************")
