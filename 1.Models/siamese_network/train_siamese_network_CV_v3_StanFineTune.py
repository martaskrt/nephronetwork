...
def process_input_image(img_file, crop=False):
    """
    ...
    # is_prep = re.findall('preprocessed', img_file)

    ## if "preprocessed" is in the image name:
    # if len(is_prep) > 0:
    if crop:
    ...
    #     image = np.array(Image.open(img_file).convert('L'))
    #     fit_img = pad_img_le(np.array(image))
    #     fit_img = torch.from_numpy(fit_img).type(torch.DoubleTensor)
    ## else:
    ## preprocess (similar/the same as how stanford data is preprocessed) ; this can be used for stanford data too
...
def get_images(in_dict, crop=False):
    img_dict = dict()
    label_dict = dict()
    cov_dict = dict()
                ...
                if len(us_nums) > 0 and surgery is not None:
                    for us_num in us_nums:
                        if in_dict[study_id][side][us_num]['Age_wks'] != "NA" and \
                            ...
                            img_dict[study_id + "_" + side + "_" + us_num]['sag'] = process_input_image(
                                in_dict[study_id][side][us_num]['sag'], crop=crop)
                            img_dict[study_id + "_" + side + "_" + us_num]['trv'] = process_input_image(
                                in_dict[study_id][side][us_num]['trv'], crop=crop)
                            ...
                            cov_dict[study_id + "_" + side + "_" + us_num]['US_machine'] = \
                            in_dict[study_id][side][us_num]['US_machine']
                            cov_dict[study_id + "_" + side + "_" + us_num]['Sex'] = in_dict[study_id]['Sex']
                            cov_dict[study_id + "_" + side + "_" + us_num]['Age_wks'] = in_dict[study_id][side][us_num][
                                'Age_wks']
                            # print(label_dict)
...
class KidneyDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, from_full_dict=True, in_dict=None, image_dict=None, label_dict=None, cov_dict=None,
                 study_ids=None,
                 cov_input=False, rand_crop=False, crop=False):
...
def train(args, train_dict, test_dict, stan_dict, max_epochs, cov_in, rand_crop=False, train_only=False):
    if train_only:
        out_file_root = args.out_dir + "/SickKids_StanFineTune_TrainOnly_" + str(max_epochs) + "epochs_bs" + \
        ...
        out_file_root = args.out_dir + "/SickKids_StanFineTune_" + str(max_epochs) + "epochs_bs" + \
                        str(args.batch_size) + "_lr" + str(args.lr) + \
                        "_RC" + str(args.random_crop) + "_cov" + str(cov_in) + "_OS" + str(args.ordered_split)
        out_file_name = out_file_root + ".txt"
    ...
    for train_tuple in id_tuples:
        ...
        if args.checkpoint != "":
            ...
            pretrained_dict = {k: v for k, v in pretrained_dict.items() if
                               k in model_dict and k not in ["fc6c.fc7.weight", "fc6c.fc7.bias", "fc7_new.fc7.weight",
                                                             "fc7_new.fc7.bias", "classifier_new.fc8.weight",
                                                             "classifier_new.fc8.bias"]}
        ...
        for epoch in range(max_epochs):
            ...
            if epoch == 0:
                path_to_checkpoint = out_file_root + '_0thEpoch.pth'
...
def main():
    ...
    parser.add_argument('--epoch_save', default=33, type=int, help="Number of epochs at which to save the model")
    ...
    parser.add_argument("--checkpoint", default="C:/Users/lauren erdman/Desktop/kidney_img/HN/SickKids/orig_st_results/SickKids_origST_TrainOnly_47epochs_bs16_lr0.001_RCTrue_covTrue_OSFalse_33Epoch.pth", help="Path to load pretrained model checkpoint from")
    ...
    parser.add_argument("--json_infile",
                        default="C:/Users/lauren erdman/Desktop/kidney_img/HN/SickKids/preprocessed_images_StanOnlyTrain_filenames_20210427.json",
                        help="Json file of model training data")
    parser.add_argument("--json_stan_test",
                        default="C:/Users/lauren erdman/Desktop/kidney_img/HN/SickKids/preprocessed_images_StanOnlyTest_filenames_20210427.json",
    ...
    if args.train_only:
        train(args, train_dict=train_dict, test_dict=None, stan_dict=stan_test_dict,
              max_epochs=args.epochs, rand_crop=args.random_crop, cov_in=args.cov_in, train_only=args.train_only)
    else:
        train(args, train_dict=train_dict, test_dict=test_dict, stan_dict=stan_test_dict,
              max_epochs=args.epochs, rand_crop=args.random_crop, cov_in=args.cov_in, train_only=args.train_only)
