...
root_dir = "C:/Users/lauren erdman/OneDrive - SickKids/HN_Stanley/"
...
length(sk_list[["STID43"]][["Right"]])
names(sk_list[["STID43"]][["Right"]])
...
for(id in names(sk_list)){
  ...
  id_sides = names(sk_list[[id]])[names(sk_list[[id]]) %in% c("Right","Left")]
  
  for(side in id_sides){
    
    pt_visits = na.omit(as.numeric(names(sk_list[[id]][[side]])))
    
    for(visits in pt_visits){
      ...
      sk_list[[id]][[side]][[visits]][["sag"]] = sub(pattern = "C:/Users/lauren erdman/Desktop/kidney_img/HN/silent_trial/ImageOutput/",
                                                         replacement = "/SilentTrial/",x = sk_list[[id]][[side]][[visits]][["sag"]])
      
      sk_list[[id]][[side]][[visits]][["trv"]] = sub(pattern = "C:/Users/lauren erdman/Desktop/kidney_img/HN/silent_trial/ImageOutput/",
                                                         replacement = "/SilentTrial/",x = sk_list[[id]][[side]][[visits]][["trv"]])
      
      sk_list[[id]][[side]][[visits]][["sag"]] = sub(pattern = "C:/Users/lauren erdman/Desktop/kidney_img/HN/SickKids/Original_CNN_Imgs/",
                                                         replacement = "/Original/",x = sk_list[[id]][[side]][[visits]][["sag"]])
      
      sk_list[[id]][[side]][[visits]][["trv"]] = sub(pattern = "C:/Users/lauren erdman/Desktop/kidney_img/HN/SickKids/Original_CNN_Imgs/",
                                                         replacement = "Original/",x = sk_list[[id]][[side]][[visits]][["trv"]])
...
test = v8()
test$assign("dat", JS(raw_in))
si_tri_list = test$get("dat")

# si_tri_revised = list()

for(id in names(si_tri_list)){

  id_sides = names(si_tri_list[[id]])[names(si_tri_list[[id]]) %in% c("Right","Left")]

  for(side in id_sides){
    
    pt_visits = na.omit(as.numeric(names(si_tri_list[[id]][[side]])))
    ...
    for(visits in pt_visits){
      ...
      si_tri_list[[id]][[side]][[visits]][["sag"]] = sub(pattern = "C:/Users/lauren erdman/Desktop/kidney_img/HN/silent_trial/ImageOutput/",
                                                         replacement = "/SilentTrial/",x = si_tri_list[[id]][[side]][[visits]][["sag"]])
      
      si_tri_list[[id]][[side]][[visits]][["trv"]] = sub(pattern = "C:/Users/lauren erdman/Desktop/kidney_img/HN/silent_trial/ImageOutput/",
                                                         replacement = "/SilentTrial/",x = si_tri_list[[id]][[side]][[visits]][["trv"]])
...
### !! UPDATE PATHS TO BE VARIABLE (EXT FROM ROOT PATH)
## find root paths

## Silent trial images: C:/Users/lauren erdman/Desktop/kidney_img/HN/silent_trial/ImageOutput/HN Outputs/
## Original data: C:/Users/lauren erdman/Desktop/kidney_img/HN/SickKids/Original_CNN_Imgs/
## stan :  C:/Users/lauren erdman/OneDrive - SickKids/HN/Stanford/RLST_labeled_imgs/
## UIowa: C:/Users/lauren erdman/OneDrive - SickKids/HN/UIowa/HN AI Images/RLST_Labeled_Images/

### get BL age groups here + double-check ages
...
for(id in unique(stan_dat$anon_mrn)){
  ...
  stan_list[[id]] = list("Left" = list(), "Right" = list())
  
  # print(unique(df_sub$`Did you indicate surgery?`))
  
  for(acc in df_sub$anon_accession){
    if(dir.exists(paste0(root_dir,"/Stanford/batch_dicom_",id,"-",acc))){
      stan_list[[id]][["Left"]][["surgery"]] = max(df_sub$l_surg_anytime[df_sub$anon_mrn == id])
      stan_list[[id]][["Right"]][["surgery"]] = max(df_sub$r_surg_anytime[df_sub$anon_mrn == id])
      stan_list[[id]][["Sex"]] = ifelse(df_sub$sex[df_sub$anon_mrn == id][1] == "M", 1, 2)
      stan_list[[id]][["BL_date"]] = "2021-01-01"
      
      if(file.exists(paste0(root_dir,"/Stanford/batch_dicom_",id,"-", acc, "/cropped/LS_cropped.jpg")) &
         file.exists(paste0(root_dir,"/Stanford/batch_dicom_",id,"-", acc, "/cropped/LT_cropped.jpg"))){
        stan_list[[id]][["Left"]][[acc]][["sag"]] = paste0("/Stanford/batch_dicom_",id,"-", acc, "/cropped/LS_cropped.jpg")
        stan_list[[id]][["Left"]][[acc]][["trv"]] = paste0("/Stanford/batch_dicom_",id,"-", acc, "/cropped/LT_cropped.jpg")
        stan_list[[id]][["Left"]][[acc]][["US_machine"]] = "Stanford"
        stan_list[[id]][["Left"]][[acc]][["ApD"]] = "NA"
        stan_list[[id]][["Left"]][[acc]][["SFU"]] = "NA"
        stan_list[[id]][["Left"]][[acc]][["Age_wks"]] = 52*df_sub$age[df_sub$anon_accession == acc]
      } else if(file.exists(paste0(root_dir,"/Stanford/batch_dicom_",id,"-", acc, "/cropped/LS_cropped.png")) &
                file.exists(paste0(root_dir,"/Stanford/batch_dicom_",id,"-", acc, "/cropped/LT_cropped.png"))){
        stan_list[[id]][["Left"]][[acc]][["sag"]] = paste0("/Stanford/batch_dicom_",id,"-", acc, "/cropped/LS_cropped.png")
        stan_list[[id]][["Left"]][[acc]][["trv"]] = paste0("/Stanford/batch_dicom_",id,"-", acc, "/cropped/LT_cropped.png")
        stan_list[[id]][["Left"]][[acc]][["US_machine"]] = "Stanford"
        stan_list[[id]][["Left"]][[acc]][["ApD"]] = "NA"
        stan_list[[id]][["Left"]][[acc]][["SFU"]] = "NA"
        stan_list[[id]][["Left"]][[acc]][["Age_wks"]] = 52*df_sub$age[df_sub$anon_accession == acc]
      ...
      if(file.exists(paste0(root_dir,"/Stanford/batch_dicom_",id,"-", acc, "/cropped/RS_cropped.jpg")) &
         file.exists(paste0(root_dir,"/Stanford/batch_dicom_",id,"-", acc, "/cropped/RT_cropped.jpg"))){
        stan_list[[id]][["Right"]][[acc]][["sag"]] = paste0("/Stanford/batch_dicom_",id,"-", acc, "/cropped/RS_cropped.jpg")
        stan_list[[id]][["Right"]][[acc]][["trv"]] = paste0("/Stanford/batch_dicom_",id,"-", acc, "/cropped/RT_cropped.jpg")
        stan_list[[id]][["Right"]][[acc]][["US_machine"]] = "Stanford"
        stan_list[[id]][["Right"]][[acc]][["ApD"]] = "NA"
        stan_list[[id]][["Right"]][[acc]][["SFU"]] = "NA"
        stan_list[[id]][["Right"]][[acc]][["Age_wks"]] = 52*df_sub$age[df_sub$anon_accession == acc]
        
      } else if(file.exists(paste0(root_dir,"/Stanford/batch_dicom_",id,"-", acc, "/cropped/RS_cropped.png")) &
                file.exists(paste0(root_dir,"/Stanford/batch_dicom_",id,"-", acc, "/cropped/RT_cropped.png"))){
        stan_list[[id]][["Right"]][[acc]][["sag"]] = paste0("/Stanford/batch_dicom_",id,"-", acc, "/cropped/RS_cropped.png")
        stan_list[[id]][["Right"]][[acc]][["trv"]] = paste0("/Stanford/batch_dicom_",id,"-", acc, "/cropped/RT_cropped.png")
        stan_list[[id]][["Right"]][[acc]][["US_machine"]] = "Stanford"
        stan_list[[id]][["Right"]][[acc]][["ApD"]] = "NA"
        stan_list[[id]][["Right"]][[acc]][["SFU"]] = "NA"
        stan_list[[id]][["Right"]][[acc]][["Age_wks"]] = 52*df_sub$age[df_sub$anon_accession == acc]
...
stan_list[["SU2bae87d"]][["Left"]]
stan_list[["SU2bae883"]][["Left"]][["surgery"]]
stan_list[["SU2bae8a1"]]


stan_list = stan_list[names(stan_list) != "SU2bae8a1"]
stan_list = stan_list[na.omit(names(stan_list))]

stan_list

write(toJSON(stan_list), "C:/Users/lauren erdman/OneDrive - SickKids/HN_Stanley/StanfordOnly_rootfilenames_20211229.json")
...
for(id in ui_dat$Name){
  ...
  if(dir.exists(paste0(root_dir,"/UIowa/",id))){
    ...
    if(ui_dat$U.B[ui_dat$Name == id] == "L"){
      ...
      ui_list[[id]][["Left"]][["1"]][["sag"]] = paste0("/UIowa/",id,"/LS_cropped.png")
      ui_list[[id]][["Left"]][["1"]][["trv"]] = paste0("/UIowa/",id,"/LT_cropped.png")
      ...
      ui_list[[id]][["Right"]][["1"]][["sag"]] = paste0("/UIowa/",id,"/RS_cropped.png")
      ui_list[[id]][["Right"]][["1"]][["trv"]] = paste0("/UIowa/",id,"/RT_cropped.png")
...
write(toJSON(ui_list), "C:/Users/lauren erdman/OneDrive - SickKids/HN_Stanley/UIonly_rootfilenames_20211229.json")
