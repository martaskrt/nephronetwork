...
wandb.init(project='VAEAC', dir="/scratch/hdd001/home/lechang", name="no translate")
...
# consistent.
def make_checkpoint():
	filename = join(args.model_dir, 'last_checkpoint_deep_t2{}.tar'.format(vlb_scale_factor ** 0.5))
	torch.save({
		'epoch': epoch,
...
for epoch in range(args.epochs):
	...
	for i, batch in enumerate(iterator):
		...
		rec_loss, kl, prior_regularization = nn.parallel.data_parallel(model, batch, device_ids=range(8)).mean()

		vlb = argsmse * rec_loss - kl + prior_regularization
	...
	with torch.no_grad():
		val_iwae, recs = get_validation_iwae(val_dataloader, mask_generator,
									   batch_size, model,
		...
		if max(validation_iwae[::-1]) <= val_iwae:
			src_filename = join(args.model_dir, 'last_checkpoint_deep_t2{}.tar'.format(vlb_scale_factor ** 0.5))
			dst_filename = join(args.model_dir, 'best_checkpoint_deep_t2{}.tar'.format(vlb_scale_factor ** 0.5))
