...
def training_loop(args, network, file_lab):
    ...
    for epoch in range(args.max_epochs):
        ...
        train_auc = roc_auc_score(np.array(flatten_list(epoch_train_lab), dtype=uint8), np.array(flatten_list(epoch_train_pred)))
        ...
        if args.include_val:
            ...
            if args.include_test:
                for idx, (us_test, lab_test) in enumerate(test_dloader):
                    ...
                    if args.dichot:
                        pred_probs_test = np.max(np.array(out_test.to("cpu").tolist()), axis=1)
                        epoch_test_pred.append(pred_probs_test)
                    else:
                        epoch_test_pred.append(out_test.to("cpu").tolist())

                if args.dichot:
                    test_auc = roc_auc_score(np.array(epoch_test_lab), np.array(epoch_test_pred))
            ...
            if args.include_test:
                for idx, (us_test, dmsa_test, lab_test) in enumerate(test_dloader):
                    ...
                    if args.dichot:
                        pred_probs_test = np.max(np.array(out_test.to("cpu").tolist()), axis=1)
                        epoch_test_pred.append(pred_probs_test.tolist())
                    else:
                        epoch_test_pred.append(out_test.to("cpu").tolist())
