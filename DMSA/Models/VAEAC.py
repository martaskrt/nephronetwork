...
class VAEAC(Module):
    ...
    def __init__(self, rec_log_prob, proposal_network, prior_network,
                 generative_network, mask_generator, sigma_mu=1e4, sigma_sigma=1e-4):
        super().__init__()
    ...
    def batch_iwae(self, batch, m, K):
        ...
        for i in range(batch.shape[0]):
            ...
            rands = random.randint(20, 70) // 2

            mask[i, 0, randx - rands: randx + rands, randy - rands: randy + rands] = 1
    ...
    def forward(self, x):
        ...
        for i in range(x.shape[0]):
            randx = random.randint(50, 472)
            randy = random.randint(50, 472)
            
            rands = random.randint(25, 70) // 2

            mask[i, 0, randx - rands: randx + rands, randy - rands: randy + rands] = 1



        return self.batch_vlb(x, mask)
