...
def process_input_image(img_file):
    ...
    # fit_img = torch.from_numpy(image).type(torch.DoubleTensor)
...
def read_image_files(sag_filename, trv_filename):
    """
    ...
    """
    # sag_img = Image.open(sag_filename).convert('L')
    # sag_arr = torch.from_numpy(np.asarray(sag_img)).type(torch.DoubleTensor)
    # sag_img.close()
    #
    # trv_img = Image.open(trv_filename).convert('L')
    # trv_arr = torch.from_numpy(np.asarray(sag_img)).type(torch.DoubleTensor)
    # trv_img.close()

    sag_arr = torch.from_numpy(special_ST_preprocessing(sag_filename)).type(torch.DoubleTensor)
    trv_arr = torch.from_numpy(special_ST_preprocessing(trv_filename)).type(torch.DoubleTensor)

    st_out = torch.cat((sag_arr, trv_arr))
...
def return_dataset(args):
    ...
    ###
    ###   @LAUREN: UPDATE THIS TO INCLUDE "age_wks" and "side + deal with sag/trans
    ###


    datasheet = pd.read_csv(args.datasheet)

    full_source_set = list(datasheet['source'].unique())
    source_set = [source for source in full_source_set if source != args.target]

    ### from my own code
    sub_keys = ["in_dict", "labels"]

    target_dict = {'train': {my_key: dict() for my_key in sub_keys},
                   'test': {my_key: dict() for my_key in sub_keys}}
    # target_test_dict = {my_key: dict() for my_key in sub_keys}
    source_dict = {'train': {source_key: {sub_key: dict() for sub_key in sub_keys} for source_key in source_set},
                   'test': {source_key: {sub_key: dict() for sub_key in sub_keys} for source_key in source_set}}
    ...
    #     target_dict[split]['in_dict'] = {'img': dict(), 'age_wks': dict(), 'side': dict()}

        # for my_key in source_dict[split].keys():
        #     source_dict[split][my_key]['in_dict'] = {'img': dict(), 'age_wks': dict(), 'side': dict()}
        #
    for index, row in datasheet.iterrows():
        ...
        if not math.isnan(row['test']):
            ...
            if row['source'] == args.target:
                ## split machine-dictionary into train and test

                # , "age_wks", "side"
                target_dict[split]["in_dict"][row['IMG_ID']] = {'img': torch.tensor(0), 'age_wks': torch.tensor(0), 'side': torch.tensor(0)}

                ## read_image_file returns 256x256 (args.dim) torch tensor
                target_dict[split]["in_dict"][row['IMG_ID']]["img"] = read_image_files(row['SAG_FILE'],
                                                                                       row['TRV_FILE']).view(2, args.dim, args.dim)

                target_dict[split]["labels"][row['IMG_ID']] = row[args.lab_col]
                target_dict[split]["in_dict"][row['IMG_ID']]["age_wks"] = row["age_wks"]
                target_dict[split]["in_dict"][row['IMG_ID']]["side"] = row["side"]

            else:
                source_dict[split][row['source']]["in_dict"][row['IMG_ID']] = {'img': torch.tensor(0), 'age_wks': torch.tensor(0), 'side': torch.tensor(0)}
                source_dict[split][row['source']]["in_dict"][row['IMG_ID']]["img"] = read_image_files(row['SAG_FILE'],
                                                                                                      row['TRV_FILE']).view(2, args.dim, args.dim)
                source_dict[split][row['source']]["labels"][row['IMG_ID']] = row[args.lab_col]
                source_dict[split][row['source']]["in_dict"][row['IMG_ID']]["age_wks"] = row["age_wks"]
                source_dict[split][row['source']]["in_dict"][row['IMG_ID']]["side"] = row["side"]

    print("Data dictionaries created.")
    # print(source_dict)
    # print(source_dict[list(source_dict.keys())])

    return source_dict['train'], source_dict['test'], target_dict['train'], target_dict['test']
...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        """
         ...
         """
        id = self.keys[index]
        in_dict, target = self.data[id], self.labels[id]

        return in_dict, target, id
...
class UnalignedDataLoader:
    def initialize(self, args, source, target, scale=32):
        ...
        for my_key in source.keys():
            self.dataset_source[my_key] = Dataset(source[my_key]['in_dict'], source[my_key]['labels'])
        self.dataset_t = Dataset(target['in_dict'], target['labels'])
...
class PairedData(object):
    ...
    def __next__(self):
        ...
        for key in self.data_loader_s.keys():
            ...
            try:
                return_dat_s[key]["in_dict"], return_dat_s[key]["label"], return_dat_s[key]["id"] = next(iter(self.data_loader_s[key]))
                # print(return_dat_s[key]['id'])
            except StopIteration:
                if return_dat_s[key]["in_dict"] is None or return_dat_s[key]["label"] is None:
                    self.stop_s[key] = True
                    self.data_loader_s_iter[key] = iter(self.data_loader_s[key])
                    return_dat_s[key]["in_dict"], return_dat_s[key]["label"], return_dat_s[key]["id"] = next(self.data_loader_s_iter[key])
        ...
        if any(self.stop_s.values()) or self.stop_t or self.iter > self.max_dataset_size:
            ...
            self.iter += 1
            out_dict = {'source': return_dat_s, 'target': {'in_dict': t, 'label': t_paths, 'id': t_ids}}
...
class SiamNet(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.cov_layers:
            ...
            # print("Age: ")
            # print(age)
            side = in_dict['side'].type(torch.FloatTensor).to(device).view(B, 1)
            # print("Side: ")
            # print(side)
...
def grad_reverse(x, lambd=1.0):
    ...
    ###
    ###   @LAUREN: UPDATE TO INCLUDE AGE AND SIDE
    ###
...
class Feature_SiamNet(nn.Module):
        ## only argument should be args
    def __init__(self, args):
        super(Feature_SiamNet, self).__init__()

        classes = 2
        self.cov_layers = args.cov_in
        self.output_dim = 256
        # print("LL DIM: " + str(self.output_dim))
        self.num_inputs = 2
    ...
    def forward(self, x):
        ...
        # if self.cov_layers:
        # in_dict = x
        # x = in_dict['img'].type(torch.FloatTensor)
...
class Predictor(nn.Module):
    def __init__(self, args, prob=0.5):
        ...
        ####
        ####        MAKE WORK WITH AGE, SIDE COVARIATES
        ####

        self.classifier_new = nn.Sequential()
        self.classifier_new.add_module('fc8', nn.Linear(args.output_dim, classes))
        self.classifier_new.add_module('relu8', nn.ReLU(inplace=True))
    ...
    def forward(self, x, age, side, reverse=False):
        # if reverse:
        #     x = grad_reverse(x, self.lambd)
        # x = F.relu(self.bn2_fc(self.fc2(x)))
        preds = self.classifier_new(x)

        age = age.type(torch.FloatTensor).to(device).view(-1, 1)
        side = side.type(torch.FloatTensor).to(device).view(-1, 1)
        mid_in = torch.cat((preds, age, side), 1)
...
class Solver(object):
    def __init__(self, args, interval=100, save_epoch=10, data_split="Train"):
        ...
        self.G = Feature_SiamNet(args=args)
        self.C1 = Predictor(args=args)
        self.C2 = Predictor(args=args)
    ...
    def train(self, epoch, record_file=None):
        ...
        for batch_idx, data in enumerate(self.datasets):
            ...
            for my_key in data['source'].keys():
                # print(my_key)
                source_data[my_key]["in_dict"]['img'] = Variable(data['source'][my_key]["in_dict"]['img'].cuda())
                source_data[my_key]["in_dict"]['age_wks'] = Variable(data['source'][my_key]["in_dict"]['age_wks'].cuda())
                source_data[my_key]["in_dict"]['side'] = Variable(data['source'][my_key]["in_dict"]['side'].cuda())
                source_data[my_key]['label'] = Variable(data['source'][my_key]['label'].long().cuda())
                source_data_too_small.append(source_data[my_key]["in_dict"]["img"].size()[0] < self.batch_size)

            target_data["in_dict"]['img'] = Variable(data['target']["in_dict"]['img'].cuda())
            target_data['label'] = Variable(data['target']['label'].long().cuda())
            target_data["in_dict"]['age_wks'] = Variable(data['target']["in_dict"]['age_wks'].long().cuda())
            target_data["in_dict"]['side'] = Variable(data['target']["in_dict"]['side'].long().cuda())
            ...
            for my_key in source_data.keys():
                source_data[my_key]["feat"] = self.G(source_data[my_key]["in_dict"]['img'])
                source_data[my_key]["output"] = self.C1(source_data[my_key]["feat"],
                                                        source_data[my_key]["in_dict"]["age_wks"],
                                                        source_data[my_key]["in_dict"]["side"])
            ...
            target_data["feat"] = self.G(target_data["in_dict"]["img"])
            target_data["output"] = self.C1(target_data["feat"],
                                            target_data["in_dict"]["age_wks"],
                                            target_data["in_dict"]["side"])
            target_data["loss"] = criterion(target_data["output"], target_data["label"])
    ...
    def test(self, epoch, record_file=None, save_model=False):
        ...
        for batch_idx, data in enumerate(self.dataset_test):
            ...
            ###
            ### @LAUREN LOOP THIS OVER KEYS AND MAKE TEST DATASET A DICTIONARY OVER ALL SOURCES
            ###

            # print(batch_idx)
            img = data['target']["in_dict"]["img"]
            label = data['target']["label"]
            id = data['target']['id']
            age = data['target']["in_dict"]['age_wks']
            side = data['target']["in_dict"]['side']

            img, label, age, side = img.cuda(), label.long().cuda(), age.cuda(), side.cuda()
            img, label, age, side = Variable(img), Variable(label), Variable(age), Variable(side)
            feat = self.G(img)
            ...
            # if epoch == self.save_epoch:
                ...
                ###
                ### @LAUREN: UPDATE GRAD CAM FUNCTION
                ###
    ...
    def feat_all_domain(self, source_dict, target_dict):
        for my_key in source_dict.keys():
            source_dict[my_key]["feat"] = self.G(source_dict[my_key]["in_dict"]["img"])
        target_dict["feat"] = self.G(target_dict["in_dict"]["img"])
    ...
    def C1_all_domain(self, source_dict, target_dict):
        for my_key in source_dict.keys():
            source_dict[my_key]["C1"] = self.C1(source_dict[my_key]["feat"],
                                                source_dict[my_key]["in_dict"]["age_wks"],
                                                source_dict[my_key]["in_dict"]["side"])
        target_dict["C1"] = self.C1(target_dict["feat"],
                                    target_dict["in_dict"]["age_wks"],
                                    target_dict["in_dict"]["side"])
        return source_dict, target_dict
    ...
    def train_MSDA(self, epoch, record_file=None):
        ...
        for batch_idx, data in enumerate(self.datasets):
            ...
            for my_key in source_data.keys():
                # print(my_key)
                # print(source_data[my_key]["in_dict"]["img"])
                source_data[my_key]["in_dict"]["img"] = Variable(source_data[my_key]["in_dict"]["img"].cuda())
                source_data[my_key]["label"] = Variable(source_data[my_key]["label"].long().cuda())
                source_data[my_key]["in_dict"]["age_wks"] = Variable(source_data[my_key]["in_dict"]["age_wks"].long().cuda())
                source_data[my_key]["in_dict"]["side"] = Variable(torch.tensor(source_data[my_key]["in_dict"]["side"])).long().cuda()
                # print(source_data[my_key]["id"])
                dat_size_lt_batch.append(source_data[my_key]["in_dict"]["img"].size()[0] < self.batch_size)

            target_data["in_dict"]["img"] = Variable(target_data["in_dict"]["img"].cuda())
            target_data["label"] = Variable(target_data["label"].long().cuda())
            target_data["in_dict"]["age_wks"] = Variable(torch.tensor(target_data["in_dict"]["age_wks"]).long().cuda())
            target_data["in_dict"]["side"] = Variable(torch.tensor(target_data["in_dict"]["side"]).long().cuda())

            if any(dat_size_lt_batch) or target_data["in_dict"]["img"].size()[0] < self.batch_size:
            ...
            target_data["feat"] = self.G(target_data["in_dict"]["img"])
            target_data["C1"] = self.C1(target_data["feat"], target_data["in_dict"]["age_wks"], target_data["in_dict"]["side"])
            target_data["C2"] = self.C2(target_data["feat"], target_data["in_dict"]["age_wks"], target_data["in_dict"]["side"])
...
def main():
    ...
    parser.add_argument('-source', default="SickKids_Stanford_UIowa",
                        help="Identifier for run")
    parser.add_argument('-target', default="TargetSample", help="data source held out of training")
    parser.add_argument('-lab_col', default="surgery", help="Name of label column in datasheet")
    ...
    parser.add_argument('-datasheet',
                        default='C:/Users/lauren erdman/Desktop/kidney_img/HN/DA/datasheets/surgery_sda_sk_st_stan_ui_subs_20210713.csv',
    ...
    parser.add_argument('-csv_outdir',
                        default='C:/Users/lauren erdman/Desktop/kidney_img/HN/DA/Output2021/csv/',
                        help="directory of DMSA images")
    parser.add_argument('-checkpoint_dir',
                        default='C:/Users/lauren erdman/Desktop/kidney_img/HN/DA/Output2021/checkpoint/',
                        help="directory of DMSA images")
    ...
    parser.add_argument('--max_epoch', type=int, default=35, metavar='N',
    ...
    parser.add_argument("-output_dim", default=256, help="M3SDA dim")
